...
public class FileRubrica {
    ...
    public Serializable remove(String cognome) {
        String telefono="";
        File mioFile=new File(sdCard,"Rubrica.txt");
        if(mioFile.exists()){
            BufferedReader mioBuffer= null;
            try {
                mioBuffer = new BufferedReader(new FileReader(mioFile));
            } catch (FileNotFoundException e) {
                e.printStackTrace();
            }
            String frase = "",fraseSplittata[];
            while(true){
                try {
                    if (!((frase=mioBuffer.readLine())!=null)) break;
                } catch (IOException e) {
                    e.printStackTrace();
                }
                fraseSplittata=frase.split(";");
